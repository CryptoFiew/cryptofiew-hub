...
const calculateMAC = (data, shortPeriod, longPeriod) => {
  if (
    !Array.isArray(data) || !Number.isFinite(shortPeriod) ||
    !Number.isFinite(longPeriod) || shortPeriod <= 0 || longPeriod <= 0
  ) {
    throw new Error(
      "Invalid parameters. Expecting an array of numbers (data) and positive numbers (shortPeriod, longPeriod).",
    );
  }
  ...
  for (let i = longPeriod - 1; i < dataLength; i++) {
    const shortSMA = calculateSMA(
      data.slice(i - shortPeriod + 1, i + 1),
      shortPeriod,
    );
    const longSMA = calculateSMA(
...
const calculateStdDev = (data) => {
  if (!Array.isArray(data) || data.length === 0) {
    throw new Error(
      "Invalid data. Expecting an array with at least one number.",
    );
  }
