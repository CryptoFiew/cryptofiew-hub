...
const detectEveningStarPattern = (candles) => {
	...
	return [false, false, ...candles.slice(2).map((currentCandle, i) => {
		...
		return previousCandle &&
			...
			Math.abs(currentCandle.open - previousCandle.close) <= (previousCandle.close - previousCandle.open) * 0.1 && // Small body
