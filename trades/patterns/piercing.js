...
const detectPiercingPattern = (candles) =>
  candles.slice(-2).reduce(([prevCandle, isPiercingPattern], currCandle) => {
    ...
    const isClosingAbovePreviousMidpoint =
      currCandle.close > (prevCandle.open + prevCandle.close) / 2;
    return [
      currCandle,
      isPiercingPattern && isOpeningBelowPreviousLow &&
      isClosingAbovePreviousMidpoint,
    ];
  }, [candles[0], true])[1];
