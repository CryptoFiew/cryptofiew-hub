...
class Logger {
    ...
    debug(message) {
        if (this.isDebug) {
            const stack = new Error().stack.split('\n').slice(3).filter(line => !line.includes('anonymous'))
...
class AuditLogger extends Logger {
	 ...
	 */
    logTrade(trade) {
        const message = `Trade - ID: ${trade.id}, Symbol: ${trade.symbol}, Volume: ${trade.volume}, Price: ${trade.price}, Date: ${trade.date}\n`
	 ...
	 */
    logOrder(order) {
        const message = `Order - ID: ${order.id}, Symbol: ${order.symbol}, Volume: ${order.volume}, Price: ${order.price}, Date: ${order.date}\n`
    ...
    logRiskCheck(riskCheck) {
        const message = `Risk Check - ID: ${riskCheck.id}, Result: ${riskCheck.result}, Date: ${riskCheck.date}\n`
