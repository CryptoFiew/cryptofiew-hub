...
const callbacks = {
  ...
  error: (error) => {
    logger.error(
      `Error connecting to WebSocket stream for symbol ${symbol}: ${error.message}`,
    );
  },
  message: (data) => {
...
const [highPrices, lowPrices] = Array.from(
  { length: priceIntervals.length },
  () =>
    Object.fromEntries(
      priceIntervals.map(
        (interval) => [interval, interval === Infinity ? -Infinity : Infinity],
      ),
    ),
);
...
function processTrade(trade) {
  ...
  rabbit.sendQueue(
    env.wsBinance,
    Buffer.from(JSON.stringify(tradeObject)),
    0,
    0,
  )
    .then(() => {
    ...
    .then(() => {
      for (const interval of priceIntervals) {
        if (timestamp % interval === 0) {
          ...
          const highPriceChannel =
            `high:${interval}:${trade.s.toLowerCase()}:binance`;
          const lowPriceChannel =
            `low:${interval}:${trade.s.toLowerCase()}:binance`;
...
function processKline(kline) {
  ...
  const intervalIndex = priceIntervals.findIndex((interval) =>
    interval === klineData.i
  );
  if (intervalIndex !== -1) {
  ...
  rabbit.sendQueue(
    env.wsBinance,
    Buffer.from(JSON.stringify(klineObject)),
    0,
    300,
    (err) => {
      if (err) {
  ...
  const channelName =
    `kline:${klineData.s.toLowerCase()}:${klineData.i}:binance`;
...
const retrieveAndStoreKlines = async (symbols) => {
  ...
  try {
    ...
    const allDataPoints = [].concat(
      ...resolvedDataPoints.map((symbolDataPoints) =>
