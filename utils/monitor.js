...
const updateTopSymbolsTicker = async () => {
	try {
		...
		const topSymbols = R.pipe(
			...
			R.filter(R.propSatisfies(R.endsWith(env.monitorSymbol.toUpperCase()), 'symbol')),
...
const processTopSymbols = async (symbols) => {
	try {
		...
		await Promise.all([
			symbolsToAdd.length > 0 ? redis.lPush(env.redisTopSymbols, ...symbolsToAdd) : null,
			symbolsToRemove.length > 0 ? redis.remove(env.redisTopSymbols, ...symbolsToRemove) : null,
		]);

		const addWatchResult = symbolsToAdd.length > 0 ? await addWatch(symbolsToAdd) : Ok();
		const delWatchResult = symbolsToRemove.length > 0 ? await delWatch(symbolsToRemove) : Ok();
