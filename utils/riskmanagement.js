...
const openPosition = (symbol, quantity, entryPrice) => {
	...
	console.log(`Opened position: ${symbol}, Quantity: ${quantity}, Entry Price: ${entryPrice}`);
};
...
const closePosition = (symbol, exitPrice) => {
	const position = positions.find((pos) => pos.symbol === symbol && pos.exitPrice === null);

	if (position) {
		position.exitPrice = exitPrice;
		position.profit = (exitPrice - position.entryPrice) * position.quantity;
		console.log(`Closed position: ${symbol}, Exit Price: ${exitPrice}, Profit: ${position.profit}`);
	} else {
